...
const sendTabMsg = (id, payload) => {
  if (isMV3) return chrome.tabs.sendMessage(id, payload).catch(() => undefined);
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    ...
    if (msg?.type === "tilt-guard/start") {
      const { settings } = await getState();
      await setCooldown(
        Date.now() + Math.max(0, Number(msg.durationMs ?? settings.durationMs))
      );
    ...
    sendResponse({ ok: false }); // fallthrough
  })();
  return true; // keep SW alive in MV3
});
